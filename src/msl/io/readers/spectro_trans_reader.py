...
class RegularTransmittanceReader(Reader):
    ...
    def can_read(file, **kwargs):
        ...
        if Reader.get_extension(file).lower() != '.dat':
            ...
            if os.path.basename(file)[0:6] == 'Trans_':
                return True
            else:
                return False
    ...
    def read(self, **kwargs):
        ...
        # lines_dat = self.get_lines(self.file, remove_empty_lines=True)
        # num_lines_dat = len(lines_dat)
        ...
        while this_line < num_lines_log - 1:
            if lines_log[this_line].startswith('Wavelength = '):
                ...
                Is.append(tuple([float(item) for item in lines_log[this_line + 7].split('|')[1:] if item]))
        ...
        fieldnames = ['wavelength', 'signal', 'u_signal', 'mon_signal', 'u_mon_signal', 'correlation', 'dark_current',
                      'u_dark_current', 'mon_dark', 'u_mon_dark']
        data = np.array(list(zip(wavelengths, I0, I0_u, M0, M0_u, corr, dark_current, dark_current_u, dark_current_M,
                                 dark_current_M_u)), dtype=[(name, float) for name in fieldnames])
        group.create_dataset('data', data=data, dtype=[(name, float) for name in fieldnames], **meta)

        fieldnames = ['Transmitted signal', 'u(Transmitted signal)', 'Monitor transmitted', 'u(Monitor transmitted)',
                      'Transmitted correlation']
        dnames = ['Is', 'Is_u', 'Is_M', 'Is_M_u', 'Is_corr']
        sample_data = np.array((list(zip(*Is)), list(zip(*Is_u)), list(zip(*Is_M)), list(zip(*Is_M_u)),
                                    list(zip(*Is_corr))))
        ind = 1
        for sample in sample_data:
            group.create_dataset(dnames[ind - 1], data=np.array(sample), dtype=[(name, float) for name in fieldnames],
                                 **meta)
            ind += 1
    ...
    def _convert_time(self, date_str:str, time_str:str):
        """
        ...
        """
        d_obj = datetime.strptime(date_str + ' ' + time_str, ' %d %B %Y %I:%M:%S %p')
        return d_obj

    def _read_celsius_file(self):
