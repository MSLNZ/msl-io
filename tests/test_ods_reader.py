...
def test_skip_rows() -> None:
    with ODSReader("tests/samples/table.ods") as ods:
        assert ods.read(sheet="A1", skip_rows=[2, 3, 6, 9]) == [
            ("timestamp", "val1", "uncert1", "val2", "uncert2"),
            (datetime(2019, 9, 11, 14, 7, 3), -0.505308, 0.000086, 0.500988, 0.000087),  # noqa: DTZ001
            (datetime(2019, 9, 11, 14, 7, 7), -0.505250, 0.000119, 0.500923, 0.000120),  # noqa: DTZ001
            (datetime(2019, 9, 11, 14, 7, 15), -0.505137, 0.000079, 0.500817, 0.000085),  # noqa: DTZ001
        ...
        assert ods.read(cells="BH11:BL21", sheet="BH11", skip_rows=[14, 17, 18]) == [
            ("timestamp", "val1", "uncert1", "val2", "uncert2"),
            (datetime(2019, 9, 11, 14, 6, 55), -0.505382, 0.000077, 0.501073, 0.000079),  # noqa: DTZ001
            (datetime(2019, 9, 11, 14, 6, 59), -0.505191, 0.000066, 0.500877, 0.000083),  # noqa: DTZ001
            (datetime(2019, 9, 11, 14, 7, 7), -0.505250, 0.000119, 0.500923, 0.000120),  # noqa: DTZ001
        ...
        assert ods.read("B,D2:E10", sheet="A1", skip_rows=[3]) == [
            (-0.505382, 0.501073, 0.000079),
            (-0.505308, 0.500988, 0.000087),
            (-0.505250, 0.500923, 0.000120),
            (-0.505275, 0.500965, 0.000088),
        ...
        assert ods.read("BI,BK12:BL20", sheet="BH11", skip_rows=[13]) == [
            (-0.505382, 0.501073, 0.000079),
            (-0.505308, 0.500988, 0.000087),
            (-0.505250, 0.500923, 0.000120),
            (-0.505275, 0.500965, 0.000088),
        ...
        assert ods.read("A:C4,E", sheet="A1", as_datetime=False, skip_rows=[4, 99, 100]) == [
            ("timestamp", "val1", "uncert1", "uncert2"),
            ("09/11/2019 14:06:55", -0.505382, 0.000077, 0.000079),
            ("09/11/2019 14:06:59", -0.505191, 0.000066, 0.000083),
        ]

        assert ods.read("BH11:BJ14,BL", sheet="BH11", as_datetime=False, skip_rows=[14, 99, 100]) == [
            ("timestamp", "val1", "uncert1", "uncert2"),
            ("09/11/2019 14:06:55", -0.505382, 0.000077, 0.000079),
            ("09/11/2019 14:06:59", -0.505191, 0.000066, 0.000083),
        ]

        assert ods.read(sheet="A1", skip_rows=range(12)) == []
        assert ods.read(cells="C4", sheet="A1") == 0.000086
        assert ods.read(cells="C4", sheet="A1", skip_rows=[4]) is None

        assert ods.read(cells="BH11:BL21", sheet="BH11", skip_rows=range(11, 22)) == []
        assert ods.read(cells="BJ14", sheet="BH11") == 0.000086
        assert ods.read(cells="BJ14", sheet="BH11", skip_rows=[14]) is None
